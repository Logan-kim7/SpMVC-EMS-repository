...
public class NaverMailSendService {
	...
	public void sendMail(EmsVO emsVO) {
		 ...
		 * 고유의 형식을 갖는다
		 * 그러한 형식을 마임타입이라고 한다.
		 * html tag로 만들어진 문서를 HTML DOC type이라고 하며
		 ...
		 * 아웃룩이 있고, 각 스마트 기기의 고유 Email App이 이런 종류에 해당한다.  
		 * 
		 */
		// 인터넷에서 사용되는 모든 문서 형식을 일컷는 것 MimeMessage
		...
		try {
			// 두번쨰 값을 true : HTML 적용을 한 메세지를 보낼수 있다. 
			// 두번째 값을 true :  메시지에 이미지 파일을 적용하여 보낼수 있다.
